...
extern TIM_HandleTypeDef htim14; // Counts packets per second.
...
volatile  uint16_t expoYaw[1024] = {0};
volatile  uint16_t expoRoll[1024] = {0};
volatile  uint16_t expoPitch[1024] = {0};
...
volatile uint8_t radioOnStartup = 1;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
	...
	joystick[gimbalsFlipped == 0 ? 3 : 2] = (
	    ...
	    channel[3] = (uint16_t)expoYaw[((map(joystick[0],(joystickMin[0][0] + 100) + trim[2], (joystickMax[0][0] - 100) + trim[2], 0, 1023) ) & 0x07FF)];

	    channel[0] = (uint16_t)expoRoll[((map(joystick[2],(joystickMin[1][0] + 100) + trim[0], (joystickMax[1][0] - 100) + trim[0], 0, 1023) ) & 0x07FF)]; // Roll
	    channel[1] = (uint16_t)expoPitch[((map(joystick[3],(joystickMin[1][1] + 100) + trim[1], (joystickMax[1][1] - 100) + trim[1], 1023, 0) ) & 0x07FF)]; // Pitch
...
void ReadMemory(int16_t* data, int size, int location){
	...
	for( int index = 0; index < size; index++){
		...
		data[index] =   (uint16_t)(*(uint64_t*)(0x0801F800UL + location)); // 128kB
...
void WriteMemory(int16_t* data, int size, int location){
	...
	epage.Page = 63; // 128kB 2k pakes. 31; // 64kB 2k pages
	epage.NbPages = 1;
	...
	for( int index = 0; index < sizeOfSettings; index++){
		...
		ret = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, 0x0801F800UL + (index * 16), memorySettings[index]); // 128kB
...
void LoRaSetup( uint8_t synchWord, uint8_t bw, uint8_t channel )
{
	...
	pRadio->SetStandby(LoRa::STDBY_RC_13MHz); // Required to change from LDO to DC-Dc Page 32 after table 5-1

	pRadio->SetRegulatorMode(1); // DC-Dc uses less current Page 32 after table 5-1.

	pRadio->SetStandby(LoRa::STDBY_XOSC_52MHz); // Switch after regulator mode selected.
	pRadio->SetPacketType(LoRa::PACKET_TYPE_LORA);

	pRadio->SetAutoFS(); // reduce switching time Page85.	radioChannel = channel;
...
//	pRadio->SetTxParams(13, 0x40);//0x40); // needs brief, power, and ramp.
	...
	// LORA_BW_400 420mW 0.49Amp 280pps.
	pRadio->SetTxParams(31, 0xe0);//0x40); // needs brief, power, and ramp.
	// LORA_BW_800 420mW 0.66Amp 510pps. shuts down.
//	pRadio->SetTxParams(20, 0x40);//0x40); // needs brief, power, and ramp.
...
void ExpoPresets()
{
	volatile float eYaw = 0.5;
	volatile float eRoll = 0.5;
	volatile float ePitch = 0.5;
	volatile float valueMult = 2.0f / 1023.0f;
	volatile float multiplier = 0.0f;
	volatile float tempYaw = 0.0f;
	volatile float tempRoll = 0.0f;
	volatile float tempPitch = 0.0f;


	for( int i = 0; i < 1024; i++)
	{
		multiplier =  abs((i * valueMult) - 1);
		tempYaw = (pow(multiplier,3) * eYaw) + ((1.0f - eYaw) * multiplier);
		tempRoll = (pow(multiplier,3) * eRoll) + ((1.0f - eRoll) * multiplier);
		tempPitch = (pow(multiplier,3) * ePitch) + ((1.0f - ePitch) * multiplier);
		if( i < 512)
		{
			expoYaw[i] = uint16_t(512.0f - (512.0f * tempYaw));
			expoRoll[i] = uint16_t(512.0f - (512.0f * tempRoll));
			expoPitch[i] = uint16_t(512.0f - (512.0f * tempPitch));
		}
		else
		{
			expoYaw[i] = uint16_t(512 + (512 * tempYaw));
			expoRoll[i] = uint16_t(512 + (512 * tempRoll));
			expoPitch[i] = uint16_t(512 + (512 * tempPitch));
		}

		expoYaw[i] = expoYaw[i] > 1023 ? 1023 : expoYaw[i];
		expoRoll[i] = expoRoll[i] > 1023 ? 1023 : expoRoll[i];
		expoPitch[i] = expoPitch[i] > 1023 ? 1023 : expoPitch[i];
	}

}
uint8_t channelToOne(uint16_t ch, uint16_t minValue)
{
	if( ch > minValue)
		return 1;
	return 0;
...
void maincpp()
{
	...
	ReadMemory(memorySettings, 20, 0);
	...
	}else if(   (TRIM_1_GPIO_Port->IDR & pinsLeft) == 0)
	{

		while((TRIM_1_GPIO_Port->IDR & pinsLeft) == 0)
		{
			HAL_Delay(60);
			LED_1_GPIO_Port->ODR ^= LED_1_Pin;
		...
		LoRa::_LoRa_BW LoRa_BW = (LoRa::_LoRa_BW)comSettings[1];

		switch(LoRa_BW)
		{
		case LoRa::LORA_BW_200:
			LED_2_GPIO_Port->BRR = LED_2_Pin;
			LED_3_GPIO_Port->BRR = LED_3_Pin;
			LED_4_GPIO_Port->BRR = LED_4_Pin;
			break;
		case LoRa::LORA_BW_400:
			LED_2_GPIO_Port->BSRR = LED_2_Pin;
			LED_3_GPIO_Port->BRR = LED_3_Pin;
			LED_4_GPIO_Port->BRR = LED_4_Pin;
			break;
		case LoRa::LORA_BW_800:
			LED_2_GPIO_Port->BSRR = LED_2_Pin;
			LED_3_GPIO_Port->BSRR = LED_3_Pin;
			LED_4_GPIO_Port->BRR = LED_4_Pin;

			break;
		case LoRa::LORA_BW_1600:
			LED_2_GPIO_Port->BSRR = LED_2_Pin;
			LED_3_GPIO_Port->BSRR = LED_3_Pin;
		...
		while(1)
		{
			...
			if( (TRIM_1_GPIO_Port->IDR & pinsLeft) != 0 && (TRIM_6_GPIO_Port->IDR & TRIM_6_Pin) == 0)
			{
				switch(LoRa_BW)
				{
				case LoRa::LORA_BW_200:
					...
					LED_3_GPIO_Port->BRR = LED_3_Pin;
					LED_4_GPIO_Port->BRR = LED_4_Pin;
					LoRa_BW = LoRa::LORA_BW_400;
					break;
				case LoRa::LORA_BW_400:
					LED_2_GPIO_Port->BSRR = LED_2_Pin;
					LED_3_GPIO_Port->BSRR = LED_3_Pin;
					LED_4_GPIO_Port->BRR = LED_4_Pin;
					LoRa_BW = LoRa::LORA_BW_800;
					break;
				case LoRa::LORA_BW_800:
					LED_2_GPIO_Port->BSRR = LED_2_Pin;
					LED_3_GPIO_Port->BSRR = LED_3_Pin;
					LED_4_GPIO_Port->BSRR = LED_4_Pin;
					LoRa_BW = LoRa::LORA_BW_1600;
				...
				while((TRIM_6_GPIO_Port->IDR & TRIM_6_Pin) == 0);
			}


			if((TRIM_1_GPIO_Port->IDR & pinsLeft) == 0 )
			{
				ReadMemory(memorySettings, 20, 0);

				// Random SynchWord from voltage value.
				memorySettings[13] = (uint16_t)((uint32_t)(batteryVoltage * 10000.0) & 0xff);
				memorySettings[14] = (uint8_t)LoRa_BW;
				memorySettings[15] = channelToOne(channel[2], 500)<<6 |
									channelToOne(channel[4], 3)<<5 |
									channelToOne(channel[5], 3)<<4 |
									channelToOne(channel[6], 3)<<3 |
									channelToOne(channel[7], 3)<<2 |
									channelToOne(channel[8], 3)<<1 |
									channelToOne(channel[9], 3)<<0;

				WriteMemory(memorySettings, 20, 0);
				...
				LoRaSetup((uint8_t)memorySettings[13],(LoRa::_LoRa_BW)memorySettings[14],(uint8_t)memorySettings[15]);
				break;
			}
			...
			if( ( TxRxTimeout == 1 || TxDone == 1) )
			{
				...
				packet[0] = (uint8_t)comSettings[0]; // SynchWord
				packet[1] = (uint8_t)comSettings[1]; // Bandwidth
				packet[2] = (uint8_t)comSettings[2]; // channel
				packet[3] = 0b10101010;
				packet[4] = 0b10101010;
				packet[5] = 0b10101010;
	...
	LoRaSetup((uint8_t)memorySettings[13],(LoRa::_LoRa_BW)memorySettings[14],(uint8_t)memorySettings[15]);
	//LoRaSetup(210, LoRa::LORA_BW_400, 50);
