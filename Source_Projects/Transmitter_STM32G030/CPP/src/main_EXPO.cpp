...
void LoRaReset()
{
	...
	pRadio->port = &hspi2;
	pRadio->Init();
	//HAL_Delay(100); // Added delay incase BUSY signal fails.
...
void ExpoPresets()
{
	volatile float eYaw = 0.6;
	volatile float eRoll = 0.6;
	volatile float ePitch = 0.6;
	...
	for( int i = 0; i < 1024; i++)
	{
		...
		//expoYaw[i] = (pow(multiplier,3) * eYaw) + ((1.0f - eYaw) * multiplier);
